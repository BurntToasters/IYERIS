...
describe('rendererSupportUi', () => {
  ...
  describe('showMigrationPopup / hideMigrationPopup', () => {
    it('shows modal and hides dismiss on first appearance', () => {
      document.body.innerHTML = `
        <div id="migration-popup-modal" style="display:none"></div>
        <button id="migration-popup-dismiss" style="display:block"></button>
      `;
      const settings = {} as any;
      const deps = makeDeps({ getCurrentSettings: () => settings });
      const ctrl = createSupportUiController(deps);
      ctrl.showMigrationPopup();

      expect(document.getElementById('migration-popup-modal')!.style.display).toBe('flex');
      expect(document.getElementById('migration-popup-dismiss')!.style.display).toBe('none');
      expect(settings.migrationNoticeViewCount).toBe(1);
      expect(deps.saveSettingsWithTimestamp).toHaveBeenCalled();
      expect(deps.activateModal).toHaveBeenCalled();
    });

    it('shows dismiss button on second appearance', () => {
      document.body.innerHTML = `
        <div id="migration-popup-modal" style="display:none"></div>
        <button id="migration-popup-dismiss" style="display:none"></button>
      `;
      const settings = { migrationNoticeViewCount: 1 } as any;
      const deps = makeDeps({ getCurrentSettings: () => settings });
      const ctrl = createSupportUiController(deps);
      ctrl.showMigrationPopup();

      expect(document.getElementById('migration-popup-dismiss')!.style.display).toBe('');
      expect(settings.migrationNoticeViewCount).toBe(2);
    });

    it('hides the migration popup modal', () => {
      document.body.innerHTML = '<div id="migration-popup-modal" style="display:flex"></div>';
      const deps = makeDeps();
      const ctrl = createSupportUiController(deps);
      ctrl.hideMigrationPopup();

      expect(document.getElementById('migration-popup-modal')!.style.display).toBe('none');
      expect(deps.deactivateModal).toHaveBeenCalled();
    });
  });

  describe('initMigrationPopup', () => {
    it('opens latest releases page from update button', () => {
      document.body.innerHTML = `
        <div id="migration-popup-modal" style="display:flex"></div>
        <button id="migration-popup-update"></button>
        <button id="migration-popup-remind"></button>
        <button id="migration-popup-dismiss"></button>
      `;

      const deps = makeDeps();
      const ctrl = createSupportUiController(deps);
      ctrl.initMigrationPopup();

      document.getElementById('migration-popup-update')!.click();
    ...
    it('remind button closes without persisting dismissal', () => {
      document.body.innerHTML = `
        <div id="migration-popup-modal" style="display:flex"></div>
        <button id="migration-popup-update"></button>
        <button id="migration-popup-remind"></button>
      ...
      document.getElementById('migration-popup-remind')!.click();

      expect(settings.migrationNoticeDismissed).toBeUndefined();
      expect(deps.saveSettingsWithTimestamp).not.toHaveBeenCalled();
      expect(document.getElementById('migration-popup-modal')!.style.display).toBe('none');
    });

    it('dont show again persists dismissal state', async () => {
      document.body.innerHTML = `
        <div id="migration-popup-modal" style="display:flex"></div>
        <button id="migration-popup-update"></button>
        <button id="migration-popup-remind"></button>
      ...
      document.getElementById('migration-popup-dismiss')!.click();

      await vi.waitFor(() => {
        expect(settings.migrationNoticeDismissed).toBe(true);
        expect(deps.saveSettingsWithTimestamp).toHaveBeenCalled();
      });
    });
  });
