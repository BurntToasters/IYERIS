...
export function setupFileAnalysisHandlers(): void {
  ...
  ipcMain.handle('calculate-folder-size', async (
    _event: IpcMainInvokeEvent,
    folderPath: string,
    operationId: string
  ): Promise<{
    success: boolean;
    result?: { totalSize: number; fileCount: number; folderCount: number; fileTypes?: { extension: string; count: number; size: number }[] };
    error?: string
  }> => {
    try {
      ...
      console.log('[FolderSize] Completed:', {
  ...
  ipcMain.handle('cancel-folder-size-calculation', async (
    _event: IpcMainInvokeEvent,
    operationId: string
  ...
  ipcMain.handle('calculate-checksum', async (
    _event: IpcMainInvokeEvent,
    filePath: string,
    operationId: string,
    algorithms: string[]
  ): Promise<{ success: boolean; result?: { md5?: string; sha256?: string }; error?: string }> => {
    try {
  ...
  ipcMain.handle('cancel-checksum-calculation', async (
    _event: IpcMainInvokeEvent,
    operationId: string
  ): Promise<{ success: boolean; error?: string }> => {
