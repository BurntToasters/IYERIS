...
describe('create-file handler', () => {
  ...
  it('creates the file when the target does not exist', async () => {
    ...
    try {
      const handler = handlers.get('create-file');
      if (!handler) {
        throw new Error('create-file handler not registered');
      }

      const result = await handler({} as any, tempDir, 'New File.txt');
      if (!result.success) {
        throw new Error(result.error || 'create-file failed');
      }
      expect(result.success).toBe(true);
      expect(result.path).toBe(path.join(tempDir, 'New File.txt'));
  ...
  it('auto-renames when the target already exists', async () => {
    ...
    try {
      ...
      expect(result.success).toBe(true);
      expect(result.path).toBe(path.join(tempDir, 'New File (2).txt'));
