export const FILE_ICON_MAP: Record<string, string> = {
  jpg: 'image',
  jpeg: 'image',
  png: 'image',
  gif: 'image',
  svg: 'image',
  bmp: 'image',
  webp: 'image',
  ico: 'image',
  tiff: 'image',
  tif: 'image',
  avif: 'image',
  jfif: 'image',
  mp4: 'file-video',
  avi: 'file-video',
  mov: 'file-video',
  mkv: 'file-video',
  webm: 'file-video',
  mp3: 'file-audio',
  wav: 'file-audio',
  flac: 'file-audio',
  ogg: 'file-audio',
  m4a: 'file-audio',
  pdf: 'file-text',
  doc: 'file-text',
  docx: 'file-text',
  txt: 'file-text',
  rtf: 'file-text',
  xls: 'file-spreadsheet',
  xlsx: 'file-spreadsheet',
  csv: 'file-spreadsheet',
  ppt: 'file-spreadsheet',
  pptx: 'file-spreadsheet',
  js: 'file-code',
  ts: 'file-code',
  jsx: 'file-code',
  tsx: 'file-code',
  html: 'file-code',
  css: 'file-code',
  json: 'file-cog',
  xml: 'file-cog',
  py: 'file-code',
  java: 'file-code',
  c: 'file-code',
  cpp: 'file-code',
  cs: 'file-code',
  php: 'file-code',
  rb: 'file-code',
  go: 'file-code',
  rs: 'file-code',
  zip: 'file-archive',
  rar: 'file-archive',
  '7z': 'file-archive',
  tar: 'file-archive',
  gz: 'file-archive',
  exe: 'binary',
  app: 'binary',
  msi: 'binary',
  dmg: 'binary',
  cr2: 'camera',
  cr3: 'camera',
  nef: 'camera',
  arw: 'camera',
  dng: 'camera',
  orf: 'camera',
  rw2: 'camera',
  pef: 'camera',
  srw: 'camera',
  raf: 'camera',
};

export const IMAGE_EXTENSIONS = new Set([
  'jpg',
  'jpeg',
  'png',
  'gif',
  'webp',
  'bmp',
  'ico',
  'tiff',
  'tif',
  'avif',
  'jfif',
  'svg',
  'apng',
  'heic',
  'heif',
  'jxl',
  'jp2',
  'j2k',
  'jpf',
  'jpx',
  'jpm',
  'mj2',
  'tga',
  'dds',
  'psd',
  'psb',
  'icns',
]);
export const RAW_EXTENSIONS = new Set([
  'cr2',
  'cr3',
  'crw',
  'nef',
  'nrw',
  'arw',
  'sr2',
  'srf',
  'srw',
  'dng',
  'orf',
  'rw2',
  'rw1',
  'rwl',
  'pef',
  'raf',
  'dcr',
  'kdc',
  'erf',
  'mrw',
  'x3f',
  '3fr',
  'iiq',
  'mef',
  'mos',
]);
export const ANIMATED_IMAGE_EXTENSIONS = new Set(['gif', 'webp', 'apng', 'avif']);
export const VIDEO_EXTENSIONS = new Set([
  'mp4',
  'm4v',
  'mov',
  'webm',
  'mkv',
  'avi',
  'ogv',
  'ogm',
  'mpg',
  'mpeg',
  'mpe',
  'm1v',
  'm2v',
  '3gp',
  '3g2',
  'ts',
  'm2ts',
  'mts',
  'flv',
  'f4v',
  'wmv',
  'asf',
  'vob',
]);
export const AUDIO_EXTENSIONS = new Set([
  'mp3',
  'mp2',
  'mpa',
  'wav',
  'flac',
  'ogg',
  'oga',
  'opus',
  'm4a',
  'm4b',
  'm4r',
  'alac',
  'aac',
  'wma',
  'aiff',
  'aif',
  'aifc',
  'mka',
  'amr',
  'ac3',
  'eac3',
  'mid',
  'midi',
  'caf',
]);
export const PDF_EXTENSIONS = new Set(['pdf']);
export const ARCHIVE_EXTENSIONS = new Set([
  'zip',
  '7z',
  'rar',
  'tar',
  'gz',
  'bz2',
  'xz',
  'iso',
  'cab',
  'arj',
  'lzh',
  'wim',
  'tgz',
]);
export const ARCHIVE_SUFFIXES = [
  '.zip',
  '.7z',
  '.rar',
  '.tar',
  '.gz',
  '.bz2',
  '.xz',
  '.iso',
  '.cab',
  '.arj',
  '.lzh',
  '.wim',
  '.tgz',
  '.tar.gz',
];
// Build TEXT_EXTENSIONS from composable sub-groups to avoid duplication
const SOURCE_CODE_EXTS = [
  'js',
  'jsx',
  'ts',
  'tsx',
  'py',
  'pyc',
  'pyw',
  'java',
  'kt',
  'kts',
  'scala',
  'c',
  'cpp',
  'cc',
  'cxx',
  'h',
  'hpp',
  'cs',
  'php',
  'rb',
  'go',
  'rs',
  'swift',
  'r',
  'lua',
  'perl',
  'pl',
  'sh',
  'bash',
  'zsh',
  'fish',
  'ps1',
  'bat',
  'cmd',
] as const;
const WEB_EXTS = ['html', 'htm', 'css', 'scss', 'sass', 'less', 'vue', 'svelte'] as const;
const DATA_EXTS = [
  'json',
  'xml',
  'yml',
  'yaml',
  'toml',
  'csv',
  'tsv',
  'ini',
  'conf',
  'config',
  'cfg',
  'env',
  'properties',
  'sql',
] as const;
const TEXT_ONLY_EXTS = [
  'txt',
  'text',
  'md',
  'markdown',
  'log',
  'readme',
  'gitignore',
  'gitattributes',
  'editorconfig',
  'dockerfile',
  'dockerignore',
  'rst',
  'tex',
  'adoc',
  'asciidoc',
  'makefile',
  'cmake',
  'gradle',
  'maven',
] as const;

export const SOURCE_CODE_EXTENSIONS = new Set<string>(SOURCE_CODE_EXTS);
export const WEB_EXTENSIONS = new Set<string>(WEB_EXTS);
export const DATA_EXTENSIONS = new Set<string>(DATA_EXTS);
export const MARKDOWN_EXTENSIONS = new Set(['md', 'markdown', 'mdown', 'mkd', 'mkdn']);
export const TEXT_EXTENSIONS = new Set<string>([
  ...SOURCE_CODE_EXTS,
  ...WEB_EXTS,
  ...DATA_EXTS,
  ...TEXT_ONLY_EXTS,
]);
export const WORD_EXTENSIONS = new Set([
  'doc',
  'docx',
  'docm',
  'dot',
  'dotx',
  'dotm',
  'rtf',
  'odt',
]);
export const SPREADSHEET_EXTENSIONS = new Set([
  'xls',
  'xlsx',
  'xlsm',
  'xlt',
  'xltx',
  'xltm',
  'ods',
  'csv',
  'tsv',
]);
export const PRESENTATION_EXTENSIONS = new Set([
  'ppt',
  'pptx',
  'pptm',
  'pps',
  'ppsx',
  'odp',
  'key',
]);
export const OFFICE_THUMBNAIL_EXTENSIONS = new Set([
  'docx',
  'docm',
  'dotx',
  'dotm',
  'xlsx',
  'xlsm',
  'xltx',
  'xltm',
  'pptx',
  'pptm',
  'ppsx',
  'odt',
  'ods',
  'odp',
]);
export const VIDEO_MIME_TYPES: Record<string, string> = {
  mp4: 'video/mp4',
  m4v: 'video/x-m4v',
  mov: 'video/quicktime',
  webm: 'video/webm',
  mkv: 'video/x-matroska',
  avi: 'video/x-msvideo',
  ogv: 'video/ogg',
  ogm: 'video/ogg',
  mpg: 'video/mpeg',
  mpeg: 'video/mpeg',
  mpe: 'video/mpeg',
  m1v: 'video/mpeg',
  m2v: 'video/mpeg',
  '3gp': 'video/3gpp',
  '3g2': 'video/3gpp2',
  ts: 'video/mp2t',
  m2ts: 'video/mp2t',
  mts: 'video/mp2t',
  flv: 'video/x-flv',
  f4v: 'video/x-f4v',
  wmv: 'video/x-ms-wmv',
  asf: 'video/x-ms-asf',
  vob: 'video/dvd',
};
export const AUDIO_MIME_TYPES: Record<string, string> = {
  mp3: 'audio/mpeg',
  mp2: 'audio/mpeg',
  mpa: 'audio/mpeg',
  wav: 'audio/wav',
  flac: 'audio/flac',
  ogg: 'audio/ogg',
  oga: 'audio/ogg',
  opus: 'audio/ogg',
  aac: 'audio/aac',
  m4a: 'audio/mp4',
  m4b: 'audio/mp4',
  m4r: 'audio/mp4',
  alac: 'audio/mp4',
  wma: 'audio/x-ms-wma',
  aiff: 'audio/aiff',
  aif: 'audio/aiff',
  aifc: 'audio/aiff',
  mka: 'audio/x-matroska',
  amr: 'audio/amr',
  ac3: 'audio/ac3',
  eac3: 'audio/eac3',
  mid: 'audio/midi',
  midi: 'audio/midi',
  caf: 'audio/x-caf',
};
