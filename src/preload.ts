...
const electronAPI: ElectronAPI = {
  ...
  searchFilesWithContent: (
    ...
    operationId?: string
  ) => ipcRenderer.invoke('search-files-content', dirPath, query, filters, operationId),
  searchFilesWithContentGlobal: (query: string, filters?: SearchFilters, operationId?: string) =>
    ipcRenderer.invoke('search-files-content-global', query, filters, operationId),
  ...
  searchIndex: (query: string, operationId?: string) =>
    ipcRenderer.invoke('search-index', query, operationId),
  ...
  cancelArchiveOperation: (operationId: string) =>
    ipcRenderer.invoke('cancel-archive-operation', operationId),
  onCompressProgress: (
    callback: (progress: {
      operationId?: string;
      current: number;
    ...
    const handler = (
      _event: Electron.IpcRendererEvent,
      progress: { operationId?: string; current: number; total: number; name: string }
    ) => callback(progress);
  ...
  cancelFolderSizeCalculation: (operationId: string) =>
    ipcRenderer.invoke('cancel-folder-size-calculation', operationId),
  onFolderSizeProgress: (
    callback: (progress: FolderSizeProgress & { operationId: string }) => void
  ) => {
    const handler = (
      _event: Electron.IpcRendererEvent,
      progress: FolderSizeProgress & { operationId: string }
    ) => callback(progress);
  ...
  onDirectoryContentsProgress: (callback: (progress: DirectoryContentsProgress) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, progress: DirectoryContentsProgress) =>
      callback(progress);
    ipcRenderer.on('directory-contents-progress', handler);
    return () => ipcRenderer.removeListener('directory-contents-progress', handler);
  },
  calculateChecksum: (filePath: string, operationId: string, algorithms: string[]) =>
    ipcRenderer.invoke('calculate-checksum', filePath, operationId, algorithms),
  cancelChecksumCalculation: (operationId: string) =>
    ipcRenderer.invoke('cancel-checksum-calculation', operationId),
  onChecksumProgress: (
    callback: (progress: { operationId: string; percent: number; algorithm: string }) => void
  ) => {
    const handler = (
      _event: Electron.IpcRendererEvent,
      progress: { operationId: string; percent: number; algorithm: string }
    ) => callback(progress);
